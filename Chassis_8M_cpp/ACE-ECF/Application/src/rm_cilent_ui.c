...
{
	HAL_UART_Transmit(&huart2,&ch,sizeof(unsigned char),1);
}
                            ...
                            Graph_Data *Graph_7)
{
    ...
    Append_CRC16_Check_Sum(msg, ALL_SEVEN_GRAPH_LEN);
  	HAL_UART_Transmit_DMA(&huart2,msg,ALL_SEVEN_GRAPH_LEN);
    UI_Seq++;
    vTaskDelay(10);
}
//只渲染一个图形(简化代码)
void My_Graph_Refresh(Graph_Data *Graph)
{
    ...
    Append_CRC16_Check_Sum(msg, ALL_GRAPH_LEN);
  	HAL_UART_Transmit_DMA(&huart2,msg,ALL_GRAPH_LEN);
...
void My_Char_Refresh(String_Data string)
{
    ...
    Append_CRC16_Check_Sum(msg, ALL_STRING_LEN);
		HAL_UART_Transmit_DMA(&huart2,msg,ALL_STRING_LEN);
